...
- Hardware-unverified. The `IDER`/`SOL` field-name mapping this module relies on for
  the `supported` signal is flagged in the [Capability matrix](capability-matrix.md) —
  read that section before trusting `supported.*` against a specific firmware.
